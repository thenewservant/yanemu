...
uint8_t* OAM = (uint8_t*)calloc(64 * 4, sizeof(uint8_t));
...
void drawSprite(uint32_t* pixels, uint8_t yy, uint8_t id, uint8_t xx, uint8_t byte2, uint8_t palette = 0) {
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
...
void showOam(uint32_t* pixels) {
	...
	for (uint8_t sprite = 0; sprite < 64; sprite++) { // sprite is OAM iterator
		if ((OAM[sprite * 4] < 0xEF) && (OAM[(3 + sprite) * 4] < 0xF9))
			drawSprite(pixels, OAM[sprite * 4], OAM[(1 + sprite) * 4], OAM[(3 + sprite) * 4], OAM[(2 + sprite) * 4]);
